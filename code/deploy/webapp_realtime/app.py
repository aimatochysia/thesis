...
def load_model(model_version='v6'):
    ...
    if model_version not in MODEL_CONFIGS:
        print("Unknown model version '{}'. Using v6 (Context-Aware CNN1D) as default.".format(model_version))
        model_version = 'v6'
...
def process_ecg(signal, fs):
    """Full pipeline: filter -> detect R-peaks -> segment -> classify (v6 only)."""
